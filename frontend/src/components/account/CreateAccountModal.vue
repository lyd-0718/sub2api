  ...
  >
    ...
    >
      ...
      >
        ...
        <!-- TLS Fingerprint -->
        <div data-testid="tls-fingerprint-control" class="rounded-lg border border-gray-200 p-4 dark:border-dark-600">
          <div class="flex items-center justify-between">
            <div>
...
const withAccountTransportExtra = <T extends Record<string, unknown> | undefined>(extra: T): T | Record<string, unknown> => {
  ...
  if (tlsFingerprintEnabled.value) {
    merged.enable_tls_fingerprint = true
    if (tlsFingerprintProfileId.value) {
      merged.tls_fingerprint_profile_id = tlsFingerprintProfileId.value
    } else {
      delete merged.tls_fingerprint_profile_id
    }
  } else {
    delete merged.enable_tls_fingerprint
    delete merged.tls_fingerprint_profile_id
  }
  return Object.keys(merged).length > 0 ? merged : extra
}
...
const handleSubmit = async () => {
  ...
  await doCreateAccount({
    ...form,
    group_ids: form.group_ids,
    extra: withAccountTransportExtra(extra),
...
const createAccountAndFinish = async (
  ...
  let finalExtra = withAccountTransportExtra(extra)
    ...
    for (let i = 0; i < refreshTokens.length; i++) {
      try {
        ...
        const createPayload = withAntigravityConfirmFlag({
          ...
          type: 'oauth',
          credentials,
          extra: withAccountTransportExtra({}),
          proxy_id: form.proxy_id,
          concurrency: form.concurrency,
          load_factor: form.load_factor ?? undefined,
          priority: form.priority,
