...
7. **CN 并发 cap 治理**（2026-09-15 新增，设计见 `PLAN-cn-cap-v5.md`）：自有文件零冲突——`service/concurrency_cap_{store,service,probe}.go`、`service/cn_quota_pause_window.go`、`service/ratelimit_classifier.go`、`service/account_error_recovery_service.go`、`repository/account_concurrency_cap_repo.go`、`handler/admin_concurrency_cap_handler.go`、迁移 `238`/`240`（新建表 + kimi 存量 >3 下调为 3）。上游文件改动：`service/concurrency_service.go`（capStore 注入 + 准入/批量夹帽，`EffectiveAccountConcurrency` 收口）；`service/ratelimit_service.go`（CN 403 分类器收口：HandleUpstreamError 403 闸门 + 幂等去重）；`service/ratelimit_cn_providers.go`（并发分支写 cap、额度分支走 `cnQuotaPauseWindow` 周满看周）；`service/gateway_forward.go` / `openai_ws_forwarder_support.go` / `openai_gateway_passthrough.go` / `openai_gateway_upstream_errors.go`（流内/WS 403 在 `openAIStream403AccountFailure` 谓词**之前**接分类器）；`service/openai_account_scheduler.go` + `handler/gateway_helper.go`（判满用夹帽值）；`service/group_capacity_service.go`（容量汇总用有效 cap）；`service/setting_update.go`（默认阈值表 +kimi:85）；`repository/scheduler_cache.go`（白名单 +15 个 CN 额度键）；`repository/account_repo.go`（+`ListCNQuotaDisabled`/`RestoreRecoveredAccount`）；`handler/handler.go`（+1 字段）；`server/routes/admin.go`（+2 路由）；`server/routes/trace_admin.go`（nil 守卫，顺带修 trace 模块既有 panic）；`config/config.go`（纯追加 `Gateway.ConcurrencyCap` 16 键）；`cmd/server/wire_gen.go`（手工接线块，注释标记，wire codegen 重跑需补回）。前端：`CreateAccountModal.vue`（kimi 初值 3）、`EditAccountModal.vue`（cap 只读展示）、`api/admin/accounts.ts` + types + zh/en i18n 各一处。
