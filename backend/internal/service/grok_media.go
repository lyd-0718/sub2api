...
func (s *OpenAIGatewayService) forwardGrokMediaVideoContent(
	...
	upstreamStart := time.Now()
	statusResp, err := doAccountHTTPUpstream(s.httpUpstream, s.tlsFPProfileService, statusReq, proxyURL, account, account.Concurrency)
	if err != nil {
		SetOpsLatencyMs(c, OpsUpstreamLatencyMsKey, time.Since(upstreamStart).Milliseconds())
		return nil, s.handleOpenAIUpstreamTransportError(ctx, c, account, err, false)
	}
	...
	contentResp, err := doAccountHTTPUpstream(s.httpUpstream, s.tlsFPProfileService, contentReq, proxyURL, account, account.Concurrency)
	SetOpsLatencyMs(c, OpsUpstreamLatencyMsKey, time.Since(upstreamStart).Milliseconds())
	if err != nil {
		return nil, s.handleOpenAIUpstreamTransportError(ctx, c, account, err, false)
	}
