...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	// 手工装配（wire codegen 重跑需补回这 3 行）：kimi Coding Plan 前缀缓存保活。
	keepaliveService := service.ProvideKeepaliveService(configConfig, accountRepository, apiKeyService, concurrencyService, openAIGatewayService)
	openAIGatewayService.SetKeepalive(keepaliveService)
	gatewayService.SetKeepalive(keepaliveService)
