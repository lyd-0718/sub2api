...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	ollamaCloudUsageService := service.ProvideOllamaCloudUsageService(accountRepository, httpUpstream, settingService, secretEncryptor, configConfig, leaderLockCache, db, tlsFingerprintProfileService)
	...
	antigravityGatewayService := service.ProvideAntigravityGatewayService(accountRepository, gatewayCache, schedulerSnapshotService, antigravityTokenProvider, rateLimitService, httpUpstream, settingService, internal500CounterCache, tlsFingerprintProfileService)
	geminiMessagesCompatService := service.ProvideGeminiMessagesCompatService(accountRepository, groupRepository, gatewayCache, schedulerSnapshotService, geminiTokenProvider, rateLimitService, httpUpstream, antigravityGatewayService, configConfig, tlsFingerprintProfileService)
	...
	grokQuotaService := service.ProvideGrokQuotaService(accountRepository, proxyRepository, grokTokenProvider, httpUpstream, configConfig, usageLogRepository, settingService, tlsFingerprintProfileService)
	...
	cnProviderQuotaService := service.ProvideCNProviderQuotaService(accountRepository, proxyRepository, httpUpstream, configConfig, tlsFingerprintProfileService)
	cnProviderBalanceService := service.ProvideCNProviderBalanceService(accountRepository, proxyRepository, httpUpstream, configConfig, tlsFingerprintProfileService)
	...
	// 二开模块：trace 管理与账号用量导出（手写接线，避免改动 wire 生成链路）。
	// 注意：若重新执行 wire 代码生成，以下 4 行需重新补上。
	traceAdminService := service.NewTraceAdminService(db)
	...
	accountErrorRecoveryService.SetTLSFingerprintProfileService(tlsFingerprintProfileService)
